...
class SplitJSONWidget(forms.Widget):
    ...
    def _as_text_field(self, name, key, value, is_sub=False):
        ...
        <input%s placeholder="%s" />""" % (attrs['id'], key, forms.util.flatatt(attrs),key)
        #return u"""
        #<input%s placeholder="%s" />""" %  (forms.util.flatatt(attrs),key)
        ...
        if l:
            ...
            for el in l:
                if isinstance(el, list) and len(l) is 1:
                    ...
                    result += '<li class="list-group-item">%s</li>' % el
            return result
        return ''
